...
export default function EvidenceDetail() {
  ...
  useEffect(() => {
    // Fetch evidence on mount instead of cases
    if (evidenceId && evidenceId !== 'new') {
        fetch(`http://localhost:8000/api/evidence/${evidenceId}`)
          .then(res => res.json())
  ...
  const runAsyncJob = async (analysisType: string, setRunning: any, setResult: any, setErrorState: any) => {
      setRunning(true);
      setErrorState(false);
      try {
          const res = await fetch(`http://localhost:8000/api/jobs/analysis/${uploadResult.id}/${analysisType}`, { method: 'POST' });
          ...
          while (!isComplete) {
              ...
              const pollRes = await fetch(`http://localhost:8000/api/jobs/${finalJob.id}`);
          ...
          if (finalJob.status === 'FAILED') {
              throw new Error(finalJob.safe_error_message || 'Analysis failed');
          }
          
          const resultRes = await fetch(`http://localhost:8000/api/analysis/${finalJob.analysis_id}`);
  ...
  const handleRunAllAnalyses = async () => {
    ...
    // Sequentially run all (Async via Jobs)
    await runAsyncJob('metadata', setAnalyzing, setAnalysisResult, setAnalysisError);
    await runAsyncJob('ela', setAnalyzingELA, setElaResult, setElaError);
    await runAsyncJob('noise', setAnalyzingNoise, setNoiseResult, setNoiseError);
    await runAsyncJob('jpeg-dct', setAnalyzingDCT, setDctResult, setDctError);
    await runAsyncJob('copy-move', setAnalyzingCopyMove, setCopyMoveResult, setCopyMoveError);
    
    // Helper to run a synchronous analysis (e.g. Fusion)
  ...
  const handleUpload = () => {
    ...
    fetch(`http://localhost:8000/api/cases/${caseId}/evidence`, {
      method: 'POST',
      body: formData
    })
      .then(async (res) => {
  ...
  const handleAnalyze = () => {
    if (!uploadResult) return;
    runAsyncJob('metadata', setAnalyzing, setAnalysisResult, setAnalysisError);
  };

  const handleAnalyzeELA = () => {
    if (!uploadResult) return;
    runAsyncJob('ela', setAnalyzingELA, setElaResult, setElaError);
  };

  const handleAnalyzeNoise = () => {
    if (!uploadResult) return;
    runAsyncJob('noise', setAnalyzingNoise, setNoiseResult, setNoiseError);
  };

  const handleAnalyzeDCT = () => {
    if (!uploadResult) return;
    runAsyncJob('jpeg-dct', setAnalyzingDCT, setDctResult, setDctError);
  };

  const handleAnalyzeCopyMove = () => {
    if (!uploadResult) return;
    runAsyncJob('copy-move', setAnalyzingCopyMove, setCopyMoveResult, setCopyMoveError);
  };
